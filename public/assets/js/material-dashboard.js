...
document.addEventListener('click', function(event) {
  ...
  const clickedOutsideSidebar = !sidenavMain.contains(event.target);
  const clickedToggleButton = toggleButton.contains(event.target);

  if (isPinned && clickedOutsideSidebar && !clickedToggleButton) {
...
document.addEventListener('DOMContentLoaded', () => {
  const dropdown = document.querySelector('.dot');
  const button = dropdown.querySelector('button');
  const dropdownMenu = dropdown.querySelector('.three-dot');  
