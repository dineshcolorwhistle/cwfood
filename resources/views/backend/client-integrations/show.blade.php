          ...
          <div class="card integration-card">
            <div class="d-flex justify-content-between align-items-start">
              <div>
                ...
                <h4 class="mb-2 tenant-name">Xero – {{ $conn->tenant_name ?? $conn->tenant_id }}</h4>
                {{-- ADD: last sync time --}}
                @if($conn->last_synced_contacts_at)
                <p class="text-muted sync-time mb-1">Last synced: {{ $conn->last_synced_contacts_at->diffForHumans() }}</p>
                @else
                <p class="text-muted sync-time mb-1">Last synced: never</p>
                @endif

                <p class="mb-2">Status: @if($conn->last_synced_status == 0)<span class="text-success">Connected</span>@else <span class="text-danger">Failed</span> @endif</p>
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header py-2">
            <h6 class="modal-title" id="miniSyncLabel"></h6>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body pt-2">
              <h5 class="text-center">Xero Syncing…</h5>
...
(function () {
  ...
  async function postSync(url, payload,card) {
    if (!modal) return;
    const el = card.querySelector('.tenant-name');
    if (el) {
        el.textContent = el.textContent.replace(/^Xero\s*[-–—]\s*/, '');
    }
    $(`#miniSyncModal .modal-title`).html(el.textContent);
    modal.show();
    const syncTime = card.querySelector('.sync-time');
    syncTime.textContent = 'Last synced: Syncing...';
    try {
  ...
  document.addEventListener('click', function (e) {
    ...
    if (!c) return; // Not clicked on a matching element

    // Step 1: Find the closest integration card wrapper
    const card = c.closest('.integration-card');
    if (!card) return;
    ...
    if (c) {
      ...
      if (c)  return postSync("{{ route('xero.sync') }}", { tenant_id: tid, full },card);
    }
  });
})();
