<script>
    ...
    $(document).ready(function() {
        ...
        function calculateLabourDetails(row) {
            ...
            // Show empty string instead of "0.00" for better UX - placeholder will show
            const hoursPerPersonInput = row.find('input[name$="[hours_per_person]"]');
            if (hoursPerPerson === 0 || hoursPerPerson === 0.00) {
                hoursPerPersonInput.val('');
            } else {
                hoursPerPersonInput.val(formatWithCommas(hoursPerPerson.toFixed(2)));
            }
        ...
        // Clear field if it's "0.00" or "0" when user focuses on it
        $(document).on('focus', 'input[name$="[hours_per_person]"]', function() {
            const $input = $(this);
            const value = removeCommas($input.val());
            const numValue = parseFloat(value);
            // If value is 0, 0.00, or empty, clear the field to show placeholder
            if (numValue === 0 || numValue === 0.00 || value === '' || value === '0' || value === '0.00') {
                $input.val('');
            }
        });
            ...
            <tr class="labour-row">
                ...
                <td>
                    <input type="text" name="LabourFields[${labourIndex}][hours_per_person]" 
                           class="form-control text-end" step="0.01" min="0" placeholder="0.00" required />
                </td>
                <td>
        ...
        $('.js-btn-save, .js-btn-next,.js-btn-finish').on('click', function () {  
            isFormChanged = true;
        });
