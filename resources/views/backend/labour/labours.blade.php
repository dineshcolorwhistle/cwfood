    ...
    <div class="">
        <div class="card-header d-flex justify-content-between">
            ...
            <div class="Export-btn">
                ...
                @if($user_role == 1)        
                <div class="btn-group click-dropdown me-2">
                    <a href="{{ route('labours.import') }}" class="btn btn-primary-orange plus-icon" title="Upload / Import data">
                        <span class="material-symbols-outlined">upload</span>
                    </a>
                </div>
                <div class="btn-group click-dropdown me-2">
                    <button type="button" class="btn btn-primary-orange plus-icon" title="Download Labour">
                        <span class="material-symbols-outlined">download</span>
                    </button>
                    <ul class="dropdown-menu">
                        <li>
                            <a class="dropdown-item export-csv" href="javascript:void(0);" data-url="/download/csv/labour">
                                Download as CSV
                            </a>
                        </li>
                        <li>
                            <a class="dropdown-item export-excel" href="javascript:void(0);" data-url="/download/excel/labour">
                                Download as Excel
                            </a>
                        </li>
                    </ul>
                ...
                @if($user_role == 1 || $user_role == 2 || $user_role == 3)
                    <button type="button" class="btn btn-primary-orange plus-icon me-2" onclick="delete_selected_labour()" title="Delete Labours">
                    <span class="material-symbols-outlined">delete</span>
                </button>
                @endif
                <input type="hidden" name="labour_number" id="labour_number" value="@if(isset($lab_code)){{$lab_code}}@endif">
                <input type="hidden" name="labour_primary" id="labour_primary" value="">
                
                @if($user_role != 4)
                <button type="button" class="btn btn-primary-orange plus-icon me-2" id="addLabourBtn" title="Add Labour">
        ...
        <div class="card-body">
            ...
            <table class="table responsiveness" id="dtRecordsView1" style="display:none;">
                <thead>
                    <tr>
                        <th class="text-primary-blue hide">Labour ID</th>
                        <th class="text-primary-blue">
                            <div class="form-check-temp p-1">
                                <input class="form-check-input raw_material_check" type="checkbox" id="labourDefault">
                            </div>
                        </th>
                        <th class="text-primary-blue">Labour Type</th>
                        <th class="text-primary-blue text-end">Hourly Rate ($)</th>
                        <th class="text-primary-blue">Category</th>
                        <th class="text-primary-blue">Notes</th>
                ...
                <tbody>
                    ...
                    @php $labour = (object) $labour; @endphp
                    <tr data-id="{{ $labour->id }}">
                        <td class="text-primary-dark-mud hide">{{ $labour->labour_id }}</td>
                        <td class="text-primary-dark-mud">
                            <div class="form-check-temp p-1">
                                <input class="form-check-input labour_check" data-labour="{{$labour->id}}" type="checkbox" id="labour_{{$labour->id}}">
                            </div>
                        </td>
                        <td class="text-primary-dark-mud">{{ ucfirst($labour->labour_type) }}</td>
                        <td class="text-primary-dark-mud text-end">{{ $labour->hourly_rate ?? '-' }}</td>
                        <td class="text-primary-dark-mud">{{ ucfirst($labour->labour_category) }}</td>
                        <td class="text-primary-dark-mud">{{ ucfirst($labour->notes) }}</td>    
                        <td class="text-primary-dark-mud hide">{{ ucfirst($labour->archive) }}</td>                                                
                            ...
                            <div class="">
                                ...
                                <div class="dropdown d-flex justify-content-end">
                                    ...
                                    <button type="button" class="icon-primary-orange me-2" title="Favourite">
                                        <span class="material-symbols-outlined" onclick="make_favorite(this)" id="makefavorite_{{$labour->id}}" data-favor="{{$labour->favorite}}" data-url="{{route('labours.favorite',['id'=>$labour->id])}}">favorite</span>
                                    </button>
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <li>
                                                <span class="dropdown-item text-primary-dark-mud me-2 edit-row-data"
                                                    data-id="{{ $labour->id }}"
                                                    data-labour-id="{{ $labour->labour_id }}"
                                                    data-labour-type="{{ $labour->labour_type }}"
                                                    data-hourly-rate="{{ $labour->hourly_rate }}"
                                                    data-labour-category="{{ $labour->labour_category }}"
                                                    data-notes="{{ $labour->notes }}">
                                                    <span class="sidenav-normal ms-2 ps-1">Edit</span>
                                                </span>
                                            </li>
                                            <li>
                                                <span class="dropdown-item text-primary-dark-mud delete-row-data" data-archive="{{ $labour->archive }}" data-id="{{ $labour->id }}">
                                                    <span class="sidenav-normal ms-2 ps-1">@if($labour->archive == 1) Delete @else Archive @endif</span>
                                                </span>
                                            </li>
                                            @if($labour->archive == 1)
                                            <li>
                                                <span class="dropdown-item text-primary-dark-mud unarchive-data" data-archive="{{ $labour->archive }}" data-id="{{ $labour->id }}">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6 form-group d-flex flex-column">
                                <label class="text-primary-orange" for="labour_type">Labour Type <span class="text-danger">*</span></label>
                                <input type="text" name="labour_type" id="labour_type" class="form-control">
                            </div>
                            <div class="col-md-6 form-group">
                                <label class="text-primary-orange" for="hourly_rate">Hourly Rate ($) <span class="text-danger">*</span></label>
                                <input type="number" name="hourly_rate" id="hourly_rate" class="form-control" step="0.01">
                            </div>
                            <div class="col-md-6 form-group d-flex flex-column">
                                <label class="text-primary-orange" for="labour_category">Labour Category</label>
                                <select name="labour_category" id="labour_category" class="form-control-select">
...
<script>
    ...
    $(document).ready(function() {
        ...
        const table = $('#dtRecordsView1').DataTable({
            ...
            initComplete: function() {
                ...
                table.columns(6).search(0, true, false).draw(); 
            ...
            if (isFormChanged) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {  
                        ...
                        $('#saveLabourBtn').click();
                        setTimeout(() => {
                            window.location.href = "{{route('labours.index')}}";    
                        }, 1000);
    ...
    $(document).on('click','#addLabourBtn', function() {
        ...
        $('#actionModalLabel').text('Add Labour');
    ...
    $(document).on('submit','#labourForm',function(e) {
        ...
        const url = labourId ?
            "{{ route('labours.update', ':id') }}".replace(':id', labourId) :
            "{{ route('labours.store') }}";
        const method = labourId ? 'POST' : 'POST';
    ...
    $(document).on('click', '.edit-row-data', function() {
        ...
        $('#labour_primary').val($(this).data('id'));
        $('#labour_id').val($(this).data('labour-id'));
        $('#labour_type').val($(this).data('labour-type'));
        $('#hourly_rate').val($(this).data('hourly-rate'));
        var labourCategory = $(this).data('labour-category').toLowerCase(); // Convert to lowercase
        $('#labour_category option').each(function() {
            if ($(this).val().toLowerCase() === labourCategory) { // Compare values in lowercase
                $(this).prop('selected', true); // Set selected option
            }
        });
        $('#notes').val($(this).data('notes'));
        $('#actionModalLabel').text('Edit Labour');
    ...
    $(document).on('click', '.delete-row-data', function() {
        ...
        const url = "{{ route('labours.destroy', ':id') }}".replace(':id', id);
        Swal.fire({
            title: 'Are you sure?',
    ...
    $(document).on('click','#labourDefault',function() {
        let selectvalue = $('#labourDefault').is(':checked')
        $('#dtRecordsView1 tbody tr').each(function() {
            $(this).find('td:eq(1) input').prop('checked',selectvalue)
        });
    });

    function delete_selected_labour(){
        let labourobj = [];
        ...
        $("table#dtRecordsView1 tbody tr").each(function () {
            if($(this).find('td:eq(1) input').prop('checked') == true){
                let id = $(this).find('td:eq(1) input').data('labour')
                labourobj.push(id)
            }
        });
        if(labourobj.length == 0){
            Swal.fire({
                icon: 'warning',
                title: 'Warning!',
                text: 'No Labour select'
            });
        }else{
            let archiveVal = $('#customFilter').val()
            let html,title,confirmBtn,inputText
            if(archiveVal == "1"){
                title = 'Delete Labours'
                confirmBtn = 'Delete'
                inputText = 'delete'
                html = `<p>The <strong>${labourobj.length}</strong> selected items will be permanently deleted and cannot be retrieved. <strong>Are you sure you want to delete them?</strong></p>
                        <p>To confirm, enter the phrase <strong>delete</strong>.</p>
                        <input id="confirmInput" class="swal2-input" placeholder="Type delete here">`
            }else{
                title = 'Archive Labours'
                confirmBtn = 'Archive'
                inputText = 'archive'
                html = `<p>The <strong>${labourobj.length}</strong> selected items will be archived. <strong>Are you sure you want to archive them?</strong></p>
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var labours = JSON.stringify(labourobj);
                    let data = {'archive':archiveVal,'labourobj':labours,'_token':$('meta[name="csrf-token"]').attr('content')}
                    $.ajax({
                        type: "POST",
                        url: "{{route('labours.delete')}}",
                        dataType: 'json',
                        data: data,
                        beforeSend: function () {},
                        success: function (response) {
    ...
    $(document).on('click','.export-csv, .export-excel', function(){
        ...
        if($('#selectedCols').val() != ''){
            ...
            let data = {'_token':$('meta[name="csrf-token"]').attr('content'),'selectedCols':selectedCols,'model':'labour'};	
            $.ajax({
                type: "POST",
    ...
    $(document).on('click','.sort_record',function(){
		...
		if (search_val == 0 || search_val == 1){
			table.columns(6).search(search_val, true, false).draw();
		}else{ 
    ...
    $(document).on('click', '.unarchive-data', function() {
        const archive = $(this).data('archive');
        const id = $(this).data('id');
        const url = "{{ route('labours.unarchive', ':id') }}".replace(':id', id);
        Swal.fire({
            title: 'Are you sure?',
