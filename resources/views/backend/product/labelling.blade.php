...
<style>
    ...
    #LabellingInformation{cursor: pointer;}
</style>
@endpush
@section('content')
    ...
    <div class="product_specs_content">
        <div class="card-header">
            <div class="top_title_wrapper d-flex justify-content-between align-items-center mb-3 hidden">
                <div class="title-add">
                    <h1>Nutritional Labeling <span class="material-symbols-outlined" id="LabellingInformation" >info</span></h1>
                </div>
            ...
            <div class="col-lg-10 col-md-10">
                <div class="row">
                    <div class="col-lg-12 col-md-12">
                        ...
                        <x-nutritional-information :product="$product" :allergen="$allergen" />
                        <x-labelling-package :prodLabel="$prodLabel"/>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-12 col-md-12">
                        <x-nutrition-analysis :product="$product" :prodIngredient="$prod_ingredient" />
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title text-primary-orange" id="actionModalLabel">Labelling Information</h2>
                </div>
                <div class="modal-body">
                    <div class="row">{!!$labelling_information!!}</div>
                </div>
...
<script>
    ...
    $(document).on('click','#pdf_download',function(){
        Swal.fire({
            title: 'Download Product Nutritional Labeling',
            html: `
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "{{ route('products.download_labelling', [$product->id, 'black']) }}";
            } else if (result.dismiss === Swal.DismissReason.cancel) {
                // Handle Custom Color download
                window.location.href = "{{ route('products.download_labelling', [$product->id, 'custom']) }}";
            }
        });
    });

    // Costing Modal Handling
    $(document).on('click','#LabellingInformation', function() {
        // Show the modal
