        ...
        <div class="card-header">
            <div class="top_title_wrapper d-flex justify-content-between">
                ...
                <div class="button-row">
                    <a href="{{ route('support.manage') }}" class="btn btn-secondary-white me-3">Back</a>
            ...
            <div class="ticket-details-section">
                ...
                <!-- <div class="row">
                    <div class="col-md-3 form-group">
                        ...
                        <select name="time_estimated" id="time_estimated" class="form-select js-example-basic-single" @if(!in_array($user_roleID,[1])) disabled @endif>
                            <option value="" disabled>Select Time Estimated</option>
                ...
                <div class="row">
                    <div class="col-md-3 form-group">
                        ...
                        <select name="time_spent" id="time_spent" class="form-select js-example-basic-single" @if(!in_array($user_roleID,[1])) disabled @endif>
                            <option value="" disabled>Select Time Spent</option>
                            @for ($i = 0.5; $i <= 8.5; $i += 0.5)
                            <option value="{{ number_format($i, 1) }}">{{ number_format($i, 1) }}h</option>
                            @endfor
                        </select>
            ...
            <div class="comment-add-section row p-4" style="display:none;">
                <div class="col-md-6"> 
                    ...
                    @csrf
                        ...
                        <div class="col-md-12 form-group">
                            <label class="text-primary-orange" for="description">Comment<span class="text-danger">*</span></label>
...
<script>
    ...
    $(window).on('load',function(){
        ...
        let time_estimated = ("{{ $ticket->time_estimated }}")?? ""
        let time_spent = ("{{ $ticket->time_spent }}")?? ""
        ...
        $('#time_estimated').val(time_estimated)
    ...
    $(document).on('submit','#supportForm',function(e){
        ...
        $.ajax({
            url: "{{ route('update.ticket', ['ticket' => $ticket->id]) }}", // The route URL
            type: "POST",
            data: formData,
                    ...
                    success: function(response) {
                        if (response.success) {
                            Swal.fire({
                                ...
                                window.location.href="{{route('support.manage')}}"
                            });
                        }else{
                            Swal.fire({
                                title: 'Error',
    ...
    $(document).on('click','#addSupportBtn', function() {
        ...
        // $('#actionModalLabel').text('Add Comment');
        // $('#saveSupportBtn').text('Save Comment');
        // // Show the modal
    ...
    $(document).on('submit','#CommentAddForm',function(e) {
        ...
        let url = "{{ route('save.comment',['ticket' => $ticket->id]) }}";     
        const method = 'POST';
    ...
    $(document).on('click', '.comment-edit', function () {
        ...
        // $('form#supportCommentForm input[name="edit_comment"]').val(desc)
