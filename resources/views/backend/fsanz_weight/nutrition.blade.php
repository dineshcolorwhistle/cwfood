...
<div class="container-fluid product-list-container fsanz_nutrition background-bg">
    <div class="card-header">
        <div class="title-add d-flex justify-content-between">
            <h1 class="page-title">FSANZ Weight</h1>
    ...
    <div class=" card-body">
        ...
        <table class="table responsiveness custom-wrap" id="dtRecordsView" style="display:none;">
            <thead class="thead-light">
                <tr>
                    <th class="text-primary-orange align-bottom">Food group ID</th>
                    <th class="text-primary-orange align-bottom">Food group name</th>
                    <th class="text-primary-orange align-bottom">Food description</th>
                    <th class="text-primary-orange align-bottom">Preparation method</th>
                    <th class="text-primary-orange align-bottom">Weight change factor</th>
                </tr>
            </thead>
            <tbody>
                @foreach($nutrition as $nutritn)
                <tr>
                    <td class="text-primary-dark-mud">{{ $nutritn->food_group_id }}</td>
                    <td class="text-primary-dark-mud">
                        @php   
                            $tooltipHtml = "<h4>{$nutritn->food_group_name}</h4> <p>{$nutritn->food_description}</p>";
                        @endphp
                        <span data-bs-toggle="tooltip" data-bs-placement="right" data-bs-custom-class="custom-tooltip admin-preference" data-bs-html="true" data-bs-title="<?= htmlspecialchars($tooltipHtml) ?>">{{ $nutritn->food_group_name }}</span>
                    </td>
                    <td class="text-primary-dark-mud">{{ $nutritn->food_description }}</td>
                    <td class="text-primary-dark-mud">{{ $nutritn->preparation_method }}</td>
                    <td class="text-primary-dark-mud text-end">{{ $nutritn->weight_change_factor }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
